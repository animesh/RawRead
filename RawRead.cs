...
namespace RawRead
{
    ...
    internal class RawRead2PeakList
    {
        static void Main(string[] args)
        {
            ...
            StreamWriter writeChromatogram = new StreamWriter(rawFile.FileName + ".intensity" + insThr + ".charge" + chgThr + "." + "chromatogram.txt");
            if (trace[0].Length > 0)
            {
            ...
            for (int i = fMS; i <= nMS; i++)
            {
                ...
                double maxInt = 0;
                string charge = "2";
                //double charge = centroidStream.Charges[i];
                if (scanStatistics.IsCentroidScan) // MS2
                {
                ...
                samples[i-1] = new Complex(scanStatistics.BasePeakMass, scanStatistics.BasePeakIntensity);
            }
